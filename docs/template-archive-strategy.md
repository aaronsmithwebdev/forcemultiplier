...
Constant Contact documents a paginated [`GET /v3/emails`](https://developer.constantcontact.com/api_guide/email-campaigns/email-campaigns-collection), campaign activity IDs, and [`GET /v3/emails/activities/{id}?include=html_content,permalink_url`](https://developer.constantcontact.com/api_guide/email-campaigns/email-campaigns-activity-id). The campaign endpoints require `campaign_data` and `campaign:read`. The app currently requests only `contact_data account_read offline_access`; a read-only campaign probe returned **403**. Enable `campaign_data` in the Constant Contact developer app, update this app's OAuth request, and reauthorize when the read-only importer is ready to use. Keep the working contact integration intact until that path is prepared.
