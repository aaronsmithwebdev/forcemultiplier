...
Read-only baseline checked 22 September 2026 and updated 24 September after the user confirmed that Constant Contact lists and unsubscribe feeds will **not** be migrated. This is historical inventory for the [email marketing platform plan](email-marketing-platform-plan.md). Counts describe connected accounts and the database at the time of the checks; no contacts, lists, campaigns, or credentials were changed.
...
| Constant Contact unsubscribe writeback   | Disabled in this app                                                                                           | Keep it disabled; use provider-neutral CSV suppression imports and prioritize Salesforce two-way opt-out sync |
...
2. Identify and mass import the approved global-unsubscribe CSV sources into ForceMultiplier. Do not start a Constant Contact unsubscribe feed or export CC list memberships for migration.
3. Confirm the authoritative Salesforce Contact opt-out field, then validate the monotonic two-way baseline and sync rules. Define Salesforce-built audiences and required marketing fields.
